...
class PredictStocks:
    ...
    def plot_prediction(self, stockname, nextsevenday, last15_predictions):
        ...
        plt.ylabel("Price",fontsize=15, color='white', fontweight=900)
        plt.title("\n" + stockname + " - Price Prediction with Historical and Predicted Data\n", fontsize=19, color='white', fontweight=900)
        ...
        today_plot_date = datetime.now().strftime('%d-%m-%Y')
        ...
        plot_path = f"{STOCKS_PLOT_PATH}/{stockname}_{current_time}_prediction_plot.jpg"
    ...
    def prediction_summary(self, stockname, nextsevenday, last15_predictions,nextday):
        ...
        # Prompt construction
        prompt = (
            f'Write a consise analysis between 2- 3 lines to explain price prediction statistically \
                mention the highest max price and lowest price, mention volitility and the slope '
                
            f"The {stockname} has a 60-day volatility of {round(vol_60, 2)} "
            f"with a predicted 7-day volatility of {round(vol_7, 2)}, marking a volatility change of {round(change_percent, 2)}%. "
            f"The maximum price in the last 60 days was {max_last60_days:,.2f}, and the minimum price was {min_last60_days:,.2f}."
            f"The 7-day slope is {round(slope, 2)}, and the next day predicted price is {nextday:,.2f}."
        )
        ...
        # print("time now  is : ",timestamp)
        summary_file_name = f"{_PATH}/{str(stockname)}_summary.txt"
        print("summary text file name :", summary_file_name )
        ...
        return response , summary_file_name 
...
"""
# this is for testing on local optional code 

if __name__ =="__main__":
    stock_model_loaded = load_model("PUT YOU MODEL PATH HERE ")
    stockname = "BTC-USD"
    Pred_Class =  PredictStocks(stockname)
    nextday, nextsevenday,last15_predictions = Pred_Class.predict_stocks( stock_model_loaded)
    response , summary_file_name  = Pred_Class.prediction_summary(stockname, nextsevenday, last15_predictions,nextday)
    print(response, summary_file_name )
